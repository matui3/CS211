package Homework.Chapter17;// Stuart Reges / Marty Stepp
...
public class IntTree {
    ...
    public int countLeftNodes() {
        return helperLeft(overallRoot);
    }

    private int helperLeft(IntTreeNode root) {
        if (root == null) {
            return 0;
        } else if (root.left != null) {
            return 1 + helperLeft(root.left) + helperLeft(root.right);
        } else {
            return helperLeft(root.right);
        }
    }
